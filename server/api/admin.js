...
router.get('/admintrips', isLoggedIn, async (req, res, next) => {
  try {
...
router.get('/admintrips/:admintripId', isLoggedIn, async (req, res, next) => {
  try {
...
router.get('/adminmessages', isLoggedIn, async (req, res, next) => {
  try {
    const user = req.user
    if (user && user.username === 'Admin') {
      const userTrips = await UserTrip.findAll({
...
router.get('/adminevents', isLoggedIn, async (req, res, next) => {
  try {
    const user = req.user
    if (user && user.username === 'Admin') {
...
router.get('/adminexpenses', isLoggedIn, async (req, res, next) => {
  try {
    const user = req.user
    if (user && user.username === 'Admin') {
      const trips = await UserTrip.findAll({
...
router.get('/adminusers', isLoggedIn, async (req, res, next) => {
  try {
    const user = req.user
    if (user && user.username === 'Admin') {
